...
const getPosition = (num: number) => {
  ...
  const col =
    row % 2 === 0
      ? (num - 1) % boardRowSize
      : boardRowSize - 1 - ((num - 1) % boardRowSize);
  return {
...
export default function GamePage() {
  ...
  const { gameId, playerId, type } = params as {
    gameId: string;
    playerId: string;
    type: string;
  };
  ...
  useEffect(() => {
    ...
    socket.on(
      ...
      async ({
        ...
        try {
          const response = await axios.get(`/api/get-winner-details`, {
            params: { gameId, userId },
          });

          if (userId === user?.id) {
            sendSolToWinner(
              response.data.winner_public_key,
              response.data.wining_amount,
              userId,
            );
          } else {
  ...
  const renderBoard = () => {
    ...
    for (let row = boardRowSize; row > 0; row--) {
      ...
      for (let col = 0; col < boardRowSize; col++) {
        const num = toggle
          ? row * boardRowSize - col
          : (row - 1) * boardRowSize + col + 1;
        rowCells.push(
          <div
            key={num}
  ...
  async function sendSolToWinner(
    winner_public_key: string,
    amount: string,
    userId: string,
  ) {
    const organizationBase58PrivateKey =
      process.env.NEXT_PUBLIC_BASE58_PRIVATE_KEY!;
    const senderKeypair = Keypair.fromSecretKey(
      bs58.decode(organizationBase58PrivateKey),
    );
    ...
    try {
      ...
      if (signature) {
        console.log(
          "Transaction successful. Explorer:",
        ...
        try {
          ...
          toast.success(
            "Congratulations! You've won double your bet. The amount has been transferred to your wallet.",
          );
          console.log("Payment details saved successfully", response);
        } catch (error) {
          if (axios.isAxiosError(error)) {
            console.log(
              "error in saving winner details",
              error.response?.data || error.message,
            );
          } else {
            console.log("Unknown error in saving winner details", error);
          }
        }
      }
    } catch (error) {
  ...
  async function updateDataForLossUser(userId: string, amount: string) {
    try {
      ...
      if (axios.isAxiosError(error)) {
        console.log(
          "error in saving payment details",
          error.response?.data || error.message,
        );
      } else {
        ...
        <div className="game-container">
          <div className="game-board-wrapper">
            <div
              style={{
                position: "relative",
                width: boardRowSize * cellSize,
                height: boardRowSize * cellSize,
              }}
            >
            ...
            <div className="game-info">
              ...
              {winner ? (
                <>
                  ...
                  <button
                    className="btn btn-yellow btn-lg d-flex align-items-center justify-content-center gap-2 mt-2 w-100"
                ...
                <div className="game-controls">
                  <button
                    className={`roll-button ${
                      players[currentTurn] !== playerId ? "disabled" : ""
                    }`}
                    onClick={rollDice}
